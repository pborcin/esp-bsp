...
static void test_eval_24bit_data(func_test_case_params_t *test_case)
{
    ...
    // Canary bytes area must stay 0
    TEST_ASSERT_EACH_EQUAL_UINT8_MESSAGE(0, (uint8_t *)test_case->buf.p_ansi, CANARY_BYTES * 3, test_msg_buf);
    TEST_ASSERT_EACH_EQUAL_UINT8_MESSAGE(0, (uint8_t *)test_case->buf.p_asm, CANARY_BYTES * 3, test_msg_buf);

    // dest_buf_asm and dest_buf_ansi must be equal
    TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE((uint8_t *)test_case->buf.p_asm + (CANARY_BYTES * 3), (uint8_t *)test_case->buf.p_ansi + (CANARY_BYTES * 3), test_case->active_buf_len * 3, test_msg_buf);

    // Canary bytes area must stay 0
    TEST_ASSERT_EACH_EQUAL_UINT8_MESSAGE(0, (uint8_t *)test_case->buf.p_ansi + ((test_case->total_buf_len * 3) - (CANARY_BYTES * 3)), CANARY_BYTES * 3, test_msg_buf);
    TEST_ASSERT_EACH_EQUAL_UINT8_MESSAGE(0, (uint8_t *)test_case->buf.p_asm + ((test_case->total_buf_len * 3) - (CANARY_BYTES * 3)), CANARY_BYTES * 3, test_msg_buf);
}
